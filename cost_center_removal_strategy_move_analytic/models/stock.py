...
class StockMove(models.Model):
    ...
    @api.model
    def _cc_removal_get_analytic_accounts_to_pass(move):
        return  move.analytic_account_id and [move.analytic_account_id.id] or None

    def action_assign(self, cr, uid, ids, context=None):
        ctx = context.copy() or {}
        for id in ids:
            move = self.pool['stock.move'].browse(cr, uid, [id], context).analytic_account_id
            ctx['cc_removal_analytics'] = self._cc_removal_get_analytic_accounts_to_pass(cr, uid, move, context=context)
